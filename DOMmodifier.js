...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //drawing tetrominos
    function draw() { //for each item in the array we add a coloring class tetromino
        currentRotation.forEach(index => {//we're coloring the divs for each tetromino
    ...
    function moveDown() {
        ...
        // it goes down, but we must limit the grid so it doesn't clip out
    }

    // we solve the clipping with a freeze function
    let takenDiv = document.createElement("div");
    takenDiv.className = "taken";
    body.appendChild(takenDiv);

    for (let i = 0; i < 10; i++) {
        takenDiv.appendChild(document.createElement("div"));
    }

    function freeze() { // if a tetromino runs into taken div, turn it to taken.
